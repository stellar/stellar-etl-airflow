...
SELECT
  ...
  IF(COALESCE(CAST(queryJobStatsTotalProcessedBytes AS INT64),
        jobStatsTotalSlotMs,
        CAST(jobStatusErrorResultCode AS INT64)) IS NULL, TRUE, FALSE
     ) AS isCached,
  /*
   * jobChange STRUCT
  ...
  STRUCT(
    ...
    STRUCT(
      ...
      STRUCT(
        ...
        REGEXP_CONTAINS(queryConfigQuery, 'cloudaudit_googleapis_com_data_access_') AS isAuditDashboardQuery,
    ...
    STRUCT(
      ...
      STRUCT(
        ...
        jobStatusErrorResultCode IS NOT NULL AS isError,
        jobStatusErrorResultMessage AS message,
        REGEXP_CONTAINS(jobStatusErrorResultMessage, 'timeout') AS isTimeout,
...
AND DATE_DIFF(CURRENT_DATE(), DATE(jobChangeEvent.jobStatsStartTime), month) <= 12
