...
def generate_update_query(schema, source_table_alias):
    ...
    update_list = ", ".join(
        [f'{field["name"]} = {source_table_alias}.{field["name"]}' for field in schema]
    )
...
def generate_equality_comparison(data_type, source_table_alias, dest_table_alias):
    ...
    if equality_comparison == "No comparison":
        raise AirflowException(
            "Unable to write query: unknown data type for merges ", data_type
        )
...
def create_merge_query(temp_table_id, data_type, schema_fields):
    ...
    equality_comparison = generate_equality_comparison(
        data_type, source_alias, dest_alias
    )
...
def apply_gcs_changes(data_type, **kwargs):
    ...
    bq_schema_list = [
        bigquery.SchemaField.from_api_repr(field) for field in schema_dict
    ]
    ...
    try:
        ...
        logging.info(
            f"Bigquery table not found; creating table with id {true_table_id}"
        )
